...
namespace PFound.NetworkLayer.Telepathy
{
    ...
    {
	    public AddressFamily AddressFamily => client == null
		                                          ? AddressFamily.Unknown
		                                          : client.Client?.AddressFamily ?? AddressFamily.Unknown;
    ...
    public class TelepathyClient : Common
    {
        // events to hook into
        ...
        public AddressFamily AddressFamily => state?.AddressFamily ?? AddressFamily.Unknown;
        ...
        // constructor
        public TelepathyClient(bool noDelay, int maxMessageSize, int sendTimeout, int receiveTimeout) : base(noDelay, maxMessageSize, sendTimeout, receiveTimeout) {}
...

        {
            ...
            try
            {
                ...
                if (isSSL)
                {
                    ...
                    state.sslStream.AuthenticateAsClient(ip);                // start send thread only after connected
                    // IMPORTANT: DO NOT SHARE STATE ACROSS MULTIPLE THREADS!
                    sendThread = new Thread(() => { ThreadFunctions.SendLoop(0, state.client, state.sslStream, state.sendPipe, state.sendPending); });
                    ...
                    ThreadFunctions.ReceiveLoop(0, state.client, state.sslStream, MaxMessageSize, state.receivePipe, ReceiveQueueLimit);
                }
                else
                {
                    // IMPORTANT: DO NOT SHARE STATE ACROSS MULTIPLE THREADS!
            ...
            catch (ObjectDisposedException disposedException)
            {
                ...
                Log.Error($"Custom Dispose Exception: {disposedException}");
            }
            catch (AuthenticationException e)
            {
                Log.Error("Client Authentication Exception: " + e);
            }
            ...
            if (state.sslStream != null)
            {
                ...
                Telepathy.Log.Info("Disposed ssl stream");
            }
            else
            {
                Telepathy.Log.Info("Skipping dispose because ssl stream is null");
            }
        ...
        {
            Telepathy.Log.Info("Validating server certificate");
            return true;
        }
        ...
        {
#if UNITY
	        Log.Info("Disconnect called on TelepathyClient");
#endif
            // only if started
            if (Connecting || Connected)
            {
#if UNITY
	            Log.Info($"Applying disconnection to TelepathyClient ClientConnectionState where Connecting: {Connecting} and Connected: {Connected}");
#endif
                // dispose all the state safely
                state.Dispose();
        ...
        private bool _isAboutToShutDownDueToSendPipeLimit = false;
        ...
        public bool Send(ArraySegment<byte> message)
        {
            if (Connected)
            {
                ...
                if (message.Count <= MaxMessageSize)
                {
                    ...
                    {
                        if (_isAboutToShutDownDueToSendPipeLimit)
                        {
                            // Don't proceed further to prevent going into infinite loop when
                            // Debugables logging tries to send the Warning below, which causes
                            // the code to reach here again.
                            return false;
                        }
                        _isAboutToShutDownDueToSendPipeLimit = true;
        ...
        public int Tick(int processLimit)
        {
            ...
            for (int i = 0; i < processLimit; ++i)
            {
                ...
                if (state.receivePipe.TryPeekForClient(out EventType eventType, out ArraySegment<byte> message))
                {
                    switch (eventType)
                    {
