...
namespace PFound.NetworkLayer.Telepathy
{
    ...
    {
        private const int _headerBufferSize = 4;
        ...
        public static bool ReadMessageBlocking(NetworkStream stream, int MaxMessageSize, byte[] headerBuffer, byte[] payloadBuffer, out int size)
        {
            ...
            if (payloadBuffer.Length != _headerBufferSize + MaxMessageSize)
            {
                Log.Error($"ReadMessageBlocking: payloadBuffer needs to be of size {_headerBufferSize} + MaxMessageSize = {_headerBufferSize + MaxMessageSize} instead of {payloadBuffer.Length}");
                return false;
            }

            // read exactly 4 bytes for header (blocking)
            if (!stream.ReadExactly(headerBuffer, _headerBufferSize))
                return false;
            ...
            Log.Warning("(Normal)ReadMessageBlocking: possible header attack with a header of: " + size + " bytes.");
            return false;
        }
        ...
        public static void ReceiveLoop(int connectionId, TcpClient client, int MaxMessageSize, MagnificentReceivePipe receivePipe, int QueueLimit)
        {
            ...
            byte[] receiveBuffer = new byte[_headerBufferSize + MaxMessageSize];

            // avoid header[4] allocations
            //
            // IMPORTANT: DO NOT make this a member, otherwise every connection
            //            on the server would use the same buffer simulatenously
            byte[] headerBuffer = new byte[_headerBufferSize];
        ...
        // (e.g. if someone has high latency or wire was cut off)
        // -> payload is of multiple <<size, content, size, content, ...> parts
        public static bool SendMessagesBlocking(SslStream stream, byte[] payload, int packetSize)
        {
            // stream.Write throws exceptions if client sends with high
            // frequency and the server stops
            try
        ...
        // read message (via stream) blocking.
        // writes into byte[] and returns bytes written to avoid allocations.
        public static bool ReadMessageBlocking(SslStream stream, int MaxMessageSize, byte[] headerBuffer, byte[] payloadBuffer, out int size)
        {
            size = 0;
            ...
            Log.Warning("(SSL)ReadMessageBlocking: possible header attack with a header of: " + size + " bytes.");
            return false;
        }

        // thread receive function is the same for client and server's clients
        public static void ReceiveLoop(int connectionId, TcpClient client, SslStream sslStream, int MaxMessageSize, MagnificentReceivePipe receivePipe, int QueueLimit)
        {
            // every receive loop needs it's own receive buffer of
            // HeaderSize + MaxMessageSize
            // to avoid runtime allocations.
            ...
            try
            {
                ...
                {
                    // read the next message (blocking) or stop if stream closed
                    if (!ReadMessageBlocking(sslStream, MaxMessageSize, headerBuffer, receiveBuffer, out int size))
                        // break instead of return so stream close still happens!
                        break;
        ...
        // note: we really do need one per connection, so that if one connection
        //       blocks, the rest will still continue to get sends
        public static void SendLoop(int connectionId, TcpClient client, SslStream sslStream, MagnificentSendPipe sendPipe, ManualResetEvent sendPending)
        {
            // avoid payload[packetSize] allocations. size increases dynamically as
            // needed for batching.
            //
            ...
            try
            {
                while (client.Connected) // try this. client will get closed eventually.
                {
                    ...
                    {
                        // send messages (blocking) or stop if stream is closed
                        if (!SendMessagesBlocking(sslStream, payload, packetSize))
                            // break instead of return so stream close still happens!
                            break;
                    }
