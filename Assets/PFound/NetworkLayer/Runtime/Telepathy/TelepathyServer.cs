...
{
    public class TelepathyServer : Common
    {

#if DEBUGABLES
        public const int ReceiveMessageSizeLimit = 64 * 1024;
        #else
        // ahmet - apo
        public const int ReceiveMessageSizeLimit = 8 * 1024;
#endif
        ...
        private Dictionary<int, IPAddress> _clientIpAddresses = new();
        ...
        // constructor
        public TelepathyServer(bool noDelay, int maxMessageSize, int sendTimeout, int receiveTimeout) : base(noDelay, maxMessageSize, sendTimeout, receiveTimeout) {}

        // the listener thread's listen function
        // note: no maxConnections parameter. high level API should handle that.
        //       (Transport can't send a 'too full' message anyway)
        void Listen(int port, bool proxyProtocolEnabled)
        {
                ...
                while (true)
                {
                    ...
                    Thread receiveThread = new Thread(() =>
                    {
                        ...
                        try
                        {
                            ...
                            {
                                byte[] proxyProtocolBuffer = new byte[108];  // 108 is the maximum length of a PROXY protocol header
                                if (client.GetStream().Read(proxyProtocolBuffer, 0, proxyProtocolBuffer.Length) == 0)
                                {
                                    Telepathy.Log.Error("Could not read PROXY protocol header.");
                                    throw new InvalidDataException("Invalid PROXY protocol header.");
                                }

                                //PROXY TCP4 178.251.45.138 127.0.0.1 8493 55010
                                string header = Encoding.ASCII.GetString(proxyProtocolBuffer);
                                if (!header.StartsWith("PROXY"))
                                {
                                    throw new InvalidDataException("Invalid PROXY protocol header.");
                                }
                                _clientIpAddresses[connectionId] = IPAddress.Parse(header.Split(' ')[2]);
                            }
                            else
                            {
                                _clientIpAddresses[connectionId] = ((IPEndPoint)client.Client.RemoteEndPoint).Address;
                            }
                            // run the receive loop
                            // (receive pipe is shared across all loops)
                            ThreadFunctions.ReceiveLoop(connectionId, client, ReceiveMessageSizeLimit, receivePipe, ReceiveQueueLimit);
        ...
        public bool Start(int port, bool proxyProtocolEnabled)
        {
            ...
            //    our new start here.
            receivePipe = new MagnificentReceivePipe(ReceiveMessageSizeLimit);
            ...
            listenerThread = new Thread(() => { Listen(port, proxyProtocolEnabled); });
        ...
        {
            if(message.Count > 60 * 1024)
                Log.Warning($"Server.Send: message too big for connection {connectionId}: " + message.Count + ". Limit: 60 * 1024");
            ...
            Log.Error($"Server.Send: message too big for connection {connectionId}: " + message.Count + ". Limit: " + MaxMessageSize);
            return false;
        }
        ...
        {
            // find the connection
            if (_clientIpAddresses.TryGetValue(connectionId, out var ipAddress))
            {
        ...
        public bool Disconnect(int connectionId)
        {
            ...
            _clientIpAddresses.Remove(connectionId, out _);
