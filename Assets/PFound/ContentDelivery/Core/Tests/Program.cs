...
namespace PFound.ContentDelivery.Core.Tests
{
    ...
    internal static class Program
    {
        ...
        private static void Ensure_DecompressesLzmaBundle(string root)
        {
            ...
            byte[] stored = Compression.Lzma.Compress(raw);   // the transferred object is the LZMA blob
        ...
        private static void Lzma_RoundTripCompressible()
        {
            ...
            byte[] comp = Compression.Lzma.Compress(data);
        ...
        private static void Lzma_PooledConcurrent()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                comps[i] = Compression.Lzma.Compress(raws[i]);
            }
            ...
            for (int t = 0; t < 64; t++)
            {
                ...
                tasks.Add(Task.Run(() =>
                {
                    ...
                    if (stream)
                    {
                        ...
                        {
                            Compression.Lzma.DecompressInto(comps[idx], ms);
                            got = ms.ToArray();
                        }
                    }
                    else got = Compression.Lzma.Decompress(comps[idx]);
        ...
        {
            byte[] back = Compression.Lzma.Decompress(Compression.Lzma.Compress(data));
